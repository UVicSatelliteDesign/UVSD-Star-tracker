...
void test_identification(database<star> data) {
	...
	for (int s = 0; s < samples; s++) {
		...
		if (visible_star_count >= 3) {
			test_orientation_from_centroids(centroids, visible_star_count, fov, quad_root, &forward, &right, &up, visible_stars, &matches, &top_three_matches);
		}
...
void test_tiled_identification(database<star> data, unsigned int subdivisions, unsigned int samples) {
	unsigned int seed = 0;
	float fov = 0.12;
	...
	for (int s = 0; s < samples; s++) {
		...
		if (visible_star_count >= 3) {
			test_atlas.get_orientation(centroids, visible_star_count, fov, fov*2, normal, &forward, &right, &up, visible_stars);
		}
...
int main() {
	...
	synthesize_database(16000, "C:/Users/logac/Desktop/UVSD Star tracker/database_16000.star");
	//database<star> data = load_database<star>("./database_16000.star");
	/*sky_atlas test_atlas(data, 10);
	for (int i = 0; i < test_atlas.tile_count; i++) {

		//std::cout << test_atlas.maps[i].star_count << std::endl;
	}
	*/
	//test_tiled_identification(data, 10, 1);
	/*
	int subdivisions = 10;
	int tiles_per_face = (subdivisions + 1) * (subdivisions + 1);
	float width = 1.0 / (subdivisions + 1);
	for (int i = 0; i < 6 * 11 * 11; i++) {
		...
		float U = 2.0 * (float(u + width / 2) / (subdivisions + 1)) - 1.0;
		float V = 2.0 * (float(v + width / 2) / (subdivisions + 1)) - 1.0;

		float hard_part = sqrt(1.0 + U * U + V * V);
		float expected = 16000.0 / (PI * tiles_per_face * hard_part * hard_part * hard_part);
		//std::cout << expected << "\n";
	}
	*/
